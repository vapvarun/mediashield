...
$countdown = (int) ( get_post_meta( $playlist_id, '_ms_countdown', true ) ?: 5 );
$loop      = (bool) get_post_meta( $playlist_id, '_ms_loop', true );
$shuffle   = (bool) get_post_meta( $playlist_id, '_ms_shuffle', true );

$first_item = $items[0];
$first_platform   = $first_item->platform ?: 'self';
$first_source_url = $first_item->source_url ?: '';
$first_protection = $first_item->protection_level ?: 'standard';
		...
		<div class="ms-playlist-items">
			...
			?>
				<div class="ms-playlist-item <?php echo 0 === $idx ? 'is-active' : ''; ?>"
					...
					data-source-url="<?php echo esc_url( $item->source_url ?: '' ); ?>"
					data-platform="<?php echo esc_attr( $item->platform ?: 'self' ); ?>"
					data-protection-level="<?php echo esc_attr( $item->protection_level ?: 'standard' ); ?>"
					...
					<div class="ms-playlist-item-info">
						...
						<span class="ms-playlist-item-platform"><?php echo esc_html( $item->platform ?: 'self' ); ?></span>
					</div>
				</div>
			<?php endforeach; ?>
		</div>
