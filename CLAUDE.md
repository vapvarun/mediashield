...
> **READ FIRST:** [`audit/manifest.summary.json`](audit/manifest.summary.json) is the index to load first (1.5 KB); [`audit/manifest.json`](audit/manifest.json) is the full inventory. Both were regenerated on **2026-08-26** by `/wp-plugin-onboard --refresh` and coverage-gated against grep ground truth, so they are no longer stale - the old warning about the 2026-05-26 manifest is gone because the manifest is current.
>
> Two categories are hand-patched because the generator cannot see them, and **must be re-patched after any regeneration**: `ms_watch_sessions_archive` (created by `str_replace()` on the sessions schema, not a literal `CREATE TABLE`) and the Setup Wizard's `add_submenu_page`. Four more (`post_types`, `capabilities`, `settings`, `wp_cli`) come back 0 from the generator and are ground-truthed in the summary. All of it is recorded under `notes.generator_blind_spots` and `notes.summary_hand_verified` - read those before trusting a count that looks wrong.
>
> **23 REST routes** in `mediashield/v1` (incl. `/wizard/complete`); WP core adds 6 more `wp/v2` routes for the two CPTs. **1 AJAX** (`ms_dismiss_pro_notice`), **2 admin pages**, **6 tables**, **3 blocks**, **3 shortcodes**, **2 CPTs**, **3 cron jobs** (2 recurring: `ms_cleanup_inactive_sessions` hourly, `ms_archive_old_sessions` monthly; 1 one-off: `ms_restore_archived_sessions` — plus the bundled EDD SDK's weekly WP-Cron licence check), **45 hooks fired** (14 actions + 31 filters, verified by scanning `do_action`/`apply_filters` in `includes/` and `src/`, excluding docblocks and line comments) plus the JS-side `mediashield_admin_routes` `wp.hooks` filter, **34 settings** (authoritative source: `array_keys( Settings::schema() )`), **1 capability** (`upload_mediashield`), **2 WP-CLI commands** (`mediashield scale`, `mediashield repair`). Service count is a manifest-only concept and **has not been re-verified for 1.3.0** — do not quote the manifest's `34` as current. **Release-ready as of the 2026-05-26 wppqa baseline** — which was taken against 1.2.0 and has NOT been re-run for 1.3.0; treat it as "no known blockers carried forward", not as a 1.3.0 sign-off — see [`audit/wppqa-baseline-2026-05-26/SUMMARY.md`](audit/wppqa-baseline-2026-05-26/SUMMARY.md) (all clean; the 2026-05-11 HIGH `nonce-no-cap` at `Menu.php:174` and the 2 MEDIUM inline-onclick findings on `VideoPostType.php` have all been resolved). See also [`audit/FEATURE_AUDIT.md`](audit/FEATURE_AUDIT.md), [`audit/CODE_FLOWS.md`](audit/CODE_FLOWS.md), [`audit/ROLE_MATRIX.md`](audit/ROLE_MATRIX.md), [`audit/derived/cross-plugin-coupling.json`](audit/derived/cross-plugin-coupling.json). Open `audit/graph.html` (`cd audit && python3 -m http.server 8765`) for an interactive Cytoscape view. Refresh via `/wp-plugin-onboard --refresh` after non-trivial changes.

> **Verifying a Ready-for-Testing card?** Follow [`docs/standards/rft-verification.md`](docs/standards/rft-verification.md) — five questions per card, a verdict format that stays comparable across the whole column, and the standing traps. The cards change every cycle; the method does not. Pre-RFT gating is the companion [`docs/standards/qa-catalog.md`](docs/standards/qa-catalog.md).
...
Verified against 1.3.0 source (`do_action` / `apply_filters` in `includes/` and `src/`, docblock and line-comment mentions excluded): **14 actions + 31 filters = 45 hooks fired by Free.**

Changed during the 1.3.0 bug sweep: `mediashield_force_output_buffer` and `mediashield_default_upload_driver` added; `mediashield_delete_remote_media` added and then removed again - remote deletion is not an opt-in any more, it does not exist. The previous figure here read 15 actions + 29 filters; the total was right but the split was not. Pro fires 20 more of its own and re-applies `mediashield_enqueue_frontend`. Two further hooks are Action Scheduler *job* hooks fired by Pro (`mediashield_generate_pdf`, `mediashield_fire_webhook`), and `mediashield_admin_routes` is a JS-side `wp.hooks` filter, not a PHP one.
