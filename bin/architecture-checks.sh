#!/usr/bin/env bash
# bin/architecture-checks.sh — MediaShield architecture-invariants gate.
#
...
#
# === Plugin-specific deviations from the universal U-set ===
# - U2 (Model base class) — N/A for MediaShield. The plugin uses CPTs
#   (mediashield_video, mediashield_playlist) plus a thin DB\Schema +
#   per-feature table accessors instead of an abstract Model base class.
#   Invariant marked `manual-review` in INVARIANTS.yaml; gate_function
#   is null and no check_U2 is registered. Do NOT silently skip — see
#   plan/INVARIANTS.yaml comment.
#
# - U3 (Base_Controller) — Adapted. There is no Base_Controller in
#   MediaShield; controllers extend WordPress's WP_REST_Controller
#   directly. check_U3 instead asserts every controller under
#   includes/REST/*Controller.php lives in the MediaShield\REST
#   namespace and extends WP_REST_Controller (or a documented subclass).
...
PLUGIN_SLUG="mediashield"
PLUGIN_TEXT_DOMAIN="mediashield"
# MODELS_GLOB / MODEL_BASE_REGEX intentionally unset — see U2 deviation note above.
CONTROLLER_GLOB="includes/REST/*Controller.php"
# MediaShield controllers extend WP_REST_Controller directly (no Base_Controller).
CONTROLLER_BASE_REGEX="extends\s+WP_REST_Controller\b"
CONTROLLER_NAMESPACE_REGEX="^namespace\s+MediaShield\\\\REST;"
...
check_U1() {
    ...
    if [ -n "$real_hits" ]; then
        violation "U1: raw \$wpdb outside DB/data layers + activate():"
        echo -e "$real_hits" | sed 's/^/    /' | head -30
    fi
}

# check_U2 intentionally NOT defined — see file-header deviation note.
# INVARIANTS.yaml marks U2 as manual-review with gate_function: null.

check_U3() {
    # Adapted: every includes/REST/*Controller.php must
    #   (a) live in the MediaShield\REST namespace, and
    #   (b) extend WP_REST_Controller (or a documented WP/Wbcom subclass).
    local violators=""
    local baselined=""
    while IFS= read -r f; do
        ...
        if grep -qE "class\s+\w*Controller\b" "$f"; then
            local ns_ok=0
            local base_ok=0
            grep -qE "$CONTROLLER_NAMESPACE_REGEX" "$f" && ns_ok=1
            grep -qE "$CONTROLLER_BASE_REGEX" "$f" && base_ok=1
            if [ "$ns_ok" -ne 1 ] || [ "$base_ok" -ne 1 ]; then
                if is_baselined U3 "$f"; then
                    baselined="${baselined}${f}\n"
                else
                    local why=""
                    [ "$ns_ok" -ne 1 ] && why="${why}wrong-namespace "
                    [ "$base_ok" -ne 1 ] && why="${why}not-WP_REST_Controller"
                    violators="${violators}${f} (${why})\n"
                fi
            fi
        fi
    done < <(find "$PLUGIN_DIR/includes/REST" -maxdepth 2 -name '*Controller.php' -type f 2>/dev/null \
    ...
    if [ -n "$violators" ]; then
        violation "U3: REST controller(s) violate namespace/base contract:"
...
check_U6() {
    ...
    if [ -n "$hits" ]; then
        ...
        if [ -n "$real_hits" ]; then
            warning "U6: asset handle(s) missing 'mediashield-' or 'ms-' prefix:"
...
# ==============================================================================
# Plugin-specific checks (none yet — author as concrete patterns emerge).
# ==============================================================================
...
echo "=== MediaShield architecture-invariants gate ==="
